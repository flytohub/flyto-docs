...
The automation engine is built from 400+ deterministic modules, composed into [YAML recipes](/guide/first-workflow), MCP-native over stdio and streamable-http, with built-in **evidence capture and replay** and human-in-the-loop control. That is not a separate technology from the war-room -- it *is* the war-room's execution layer:
