...
The controls this surface checks for are the controls we hardened on our own MCP server. [flyto-core](/) is itself an MCP server exposing 452 modules over [stdio and streamable HTTP](/mcp/) — so the same three capability classes apply to it, and we fixed them on ourselves first:
