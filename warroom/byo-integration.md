...
2. **Connect domains / asset map** — external attack surface + asset inventory
3. **Ingest external feeds** — external-rating, darkweb, threat-intel
...
If you already maintain an asset inventory elsewhere, this is a BYO step too: import your known hosts so discovery *augments* your inventory rather than starting from zero. What you bring is treated as ground truth; what we discover is the supplement.

## Step 3 — Ingest external feeds
...
### The no-double-charge principle

We charge for **integration and the closed loop, not for re-running an algorithm you already paid for.** If you already own an external-rating product, we ingest its output and use it as a signal in correlation — we do not re-rate from scratch and bill you for a number you already have. Your existing investment becomes an input to the unified picture. What you pay Flyto2 for is the convergence: pulling nine surfaces into one score, one timeline, and one evidence trail, then driving that through pentest and red-team.
...
- **CI-side uploads** — if you already run scanners in your pipeline, post their results in with your API key instead of re-scanning in the platform:
