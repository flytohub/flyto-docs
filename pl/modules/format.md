...
| Name | Type | Required | Default | Description |
|------|------|----------|---------|-------------|
| `value` | number | Yes | - | Wartość do sformatowania jako procent |
| `is_ratio` | boolean | No | `True` | Wartość do sformatowania jako procent |
| `decimal_places` | number | No | `1` | Wejście to współczynnik (0-1), który należy pomnożyć przez 100 |
| `include_sign` | boolean | No | `False` | Liczba miejsc dziesiętnych |

**Output:**

| Field | Type | Description |
|-------|------|-------------|
| `result` | string | Uwzględnij znak + dla wartości dodatnich |
| `original` | number | Sformatowany ciąg procentowy |
| `numeric` | number | Sformatowany ciąg procentowy |
